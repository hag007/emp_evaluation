...
def dot_plot(df_measurements, y_label, filter_zeros=False, ax=None, algos=None, title=""):
    ...
    ax = sns.violinplot(x='Alg', y=y_label, data=df_new, jitter=True, size=15, ax=ax, order=my_order, palette={a: "#AAAAAA" for a in my_order}) # {a: colorlist[algo_acrs.index(a)] for a in my_order}
...
def main():
    ...
    fig_4.tight_layout()
    fig_4.savefig(os.path.join(constants.OUTPUT_GLOBAL_DIR, "figure_4_v.png"))

    fig_5.text(0.01, 0.97, "A:", weight='bold', fontsize=22)
    fig_5.text(0.5, 0.97, "B:", weight='bold', fontsize=22)
    fig_5.tight_layout()
    fig_5.savefig(os.path.join(constants.OUTPUT_GLOBAL_DIR, "figure_5_v.png"))
