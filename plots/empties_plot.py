...
if __name__=="__main__":
    ...
    for prefix, ax, algos in zip(prefixes, axs,
        ...
        for idx, values in df_means.iterrows():
            ax.plot([0.6, 0.7, 0.8, 0.9], values, linestyle=(list(df_means.index).index(idx)*4,(4,6)), linewidth=3.0, c=constants.COLORDICT[idx], label=constants.ALGOS_ACRONYM[idx])
        ...
        patches=[Line2D([0], [0], linestyle='--',  color=constants.COLORDICT[a], label=constants.ALGOS_ACRONYM[a], markersize=12, markerfacecolor=constants.COLORDICT[a], alpha=0.7) for a in algos]
        # for cur in patches;
        #     cur.set_dashes()
