...
def false_positive_example(algo="jactivemodules_greedy",dataset="TNFa_2", results=None):
    ...
    filtered_GO_terms=open(os.path.join(constants.GO_DIR,"filtered_go_terms.txt"), 'r').read().split()
    output=pd.read_csv(os.path.join(constants.OUTPUT_GLOBAL_DIR,"bg","emp_diff_modules_{}_{}.tsv".format(dataset, algo)), sep='\t', index_col=0, error_bad_lines=False).loc[filtered_GO_terms]
    output_md = pd.read_csv(os.path.join(constants.OUTPUT_GLOBAL_DIR,"md", "emp_diff_modules_{}_{}_md.tsv".format(dataset, algo)), sep='\t', index_col=0, error_bad_lines=False)
#     if not "GO id" in output_md.columns:
#         df_summary.loc[dataset, algo]="0/0=0"
#         return


    output=output.loc[output.index.isin(output_md.index.values)]

    real_pvals=output_md.loc[output_md.loc[:,"hg_pval_max"].apply(lambda a: isFloat(a))].dropna()
    real_pvals["hg_pval_max"]=real_pvals["hg_pval_max"].values.astype(np.float)
    real_sig_ids = real_pvals[fdrcorrection0([10**-a for a in real_pvals["hg_pval_max"]], alpha=0.05, method='indep', is_sorted=False)[0]].index
    ...
    for cur_bg in np.arange(n_of_bg):
        ...
        if (output.shape[0]!=0):
            ...
            bg_sig_ids=bg_pvals[fdrcorrection0([10**-a for a in  bg_pvals["cur_bg_hg_pval"]], alpha=0.05, method='indep', is_sorted=False)[0]].index
