...
def plot_modules_ehr_summary(prefix, datasets, algos):
    ...
    df_statistics.to_csv(
        os.path.join(constants.OUTPUT_GLOBAL_DIR, "mehr_cache_files", "modules_statistics_{}.tsv".format(prefix)),
        sep='\t')
    ...
    for i_a, cur_alg in enumerate(set(constants.ALGOS).intersection(algos)):
        for i_d, cur_ds in enumerate(datasets):
            ...
            ax_main.plot([-0.5, 13.5], [(len(datasets)) * i_a - 0.3, (len(datasets)) * i_a - 0.3],
                         linestyle=(0, (5, 10)), linewidth=1, c='black')
    ...
    for n_modules_th in range(1, MAX_N_MODULES_TH + 1):
        ...
        for cur_ds in datasets:
            for cur_alg in algos:
                id = "{}_{}".format(cur_alg, cur_ds)
                y_axis += 1
                if id not in list(df_statistics.index):
    ...
    for i in np.arange(MAX_N_MODULES_TH / 2):
        ...
        df_mEHR.to_csv(os.path.join(constants.OUTPUT_GLOBAL_DIR, "mehr_cache_files",
                                    "summary_mEHR_mean_{}_{}.tsv".format(i + 1, prefix)), sep='\t')
...
if __name__ == "__main__":
    ...
    parser.add_argument('--datasets', dest='datasets',
                        default="Breast_Cancer.G50,Crohns_Disease.G50,Schizophrenia.G50,Triglycerides.G50,Type_2_Diabetes.G50")  # TNFa_2,HC12,SHERA,SHEZH_1,ROR_1,ERS_1,IEM Breast_Cancer.G50,Crohns_Disease.G50,Schizophrenia.G50,Triglycerides.G50,Type_2_Diabetes.G50
