...
if __name__ == "__main__":
    ...
    for cur_ds in datasets:
        ...
        sig_hg_genes_flatted=reduce(lambda x, y: x + y, sig_hg_genes_list, [])
        sig_emp_genes_flatted=reduce(lambda x, y: x + y, sig_emp_genes_list, [])

        hg_ratio=len(set(sig_hg_genes_flatted))/float(len(sig_hg_genes_flatted))
        emp_ratio=len(set(sig_emp_genes_flatted))/float(len(sig_emp_genes_flatted))

        print "cur dataset: {}, HG ratio: {}".format(cur_ds, hg_ratio)
        print "cur dataset: {}, EMP ratio: {}".format(cur_ds, emp_ratio)
        df_summary.loc[cur_ds, "HG_union"]=len(set(sig_hg_genes_flatted))
        df_summary.loc[cur_ds, "HG_intersection"] = len(set(sig_hg_genes_list[0]).intersection(*sig_hg_genes_list))
        df_summary.loc[cur_ds, "HG_count"]=len(sig_hg_genes_list)
        df_summary.loc[cur_ds, "HG_ratio"]=hg_ratio
        df_summary.loc[cur_ds,"EMP_union"]=len(set(sig_emp_genes_flatted))
        df_summary.loc[cur_ds, "EMP_intersection"] = len(set(sig_emp_genes_list[0]).intersection(*sig_emp_genes_list))
        df_summary.loc[cur_ds, "EMP_count"]=len(sig_emp_genes_flatted)
        df_summary.loc[cur_ds, "EMP_ratio"]=emp_ratio


    df_summary.to_csv(os.path.join(constants.OUTPUT_GLOBAL_DIR, "terms_union_ratio.tsv"), sep='\t')
