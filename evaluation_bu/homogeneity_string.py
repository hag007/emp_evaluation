...
def calc_homogeneity(cache_file, dataset=None, algo=None, module_i=None, base_folder=None, cutoff=1, module=[], pf=3, file_format = None, sim_method="Resnik"):
...
def main(prefix, base_folder, sim_method, file_format, pf, datasets, algos, cutoffs, recalc_module_report):
        ...
        for cur_ds in datasets:
            ...
            for cur_alg in algos:
                ...
                for cur_module_index in module_indices:
                    ...
                    params.append([calc_homogeneity, [cache_file, cur_ds, cur_alg, cur_module_index, base_folder, cutoff, go_set, 3, file_format]])

    p=MyPool(pf)
    results=p.map(func_star, params)
    p.close()
    for cur_res in results:
        h_scores[cur_res[0]].loc["{}_{}_modules_{}".format(cur_res[1], cur_res[2], cur_res[3]), "homogeneity_score"]=cur_res[4]
    ...
    for cutoff in cutoffs:
        ...
        for cur_ds in datasets:
            for cur_alg in algos:
                ds_filtered_scores=h_scores[cutoff].loc[(h_scores[cutoff]['algo']== cur_alg).values & (h_scores[cutoff]['dataset']== cur_ds).values]
        ...
        h_scores[cutoff].to_csv(os.path.join(constants.OUTPUT_GLOBAL_DIR, "evaluation", "h_scores_{}_{}.tsv".format(prefix,str(cutoff))), sep='\t')
...
if __name__ == "__main__":
    ...
    algos=["DOMINO4","netbox2_string"] # ["DOMINO", "netbox", "jactivemodules_greedy", "jactivemodules_sa", "bionet", "keypathwayminer_INES_GREEDY", "hotnet2"]
    prefix="GE"
