...
def main(dataset="SOC", algo="jactivemodules_sa", n_permutations=300,
    ...
    df_deleted=df.drop(df.dropna().index,axis=0)
    df = df.dropna()
    ...
    print "{}".format(np.size(max_genes_pvals))
    ...
    df_all = pd.concat((df_filtered_in, df_filtered_out, df_deleted), axis=0)
    print df_filtered_in.columns, df_all.columns
    df_all.loc[:,
        ["GO name", "hg_pval", "hg_pval_max", "emp_pval", "hg_rank", "emp_rank", "n_genes", "depth", "passed_fdr"]].to_csv(csv_file_name[:-4] + "_md.tsv", sep='\t') # [df_all["hg_pval"].values > 0, :]
    return len(df_filtered_in.index), max_true_counter, HG_CUTOFF, emp_cutoff


if __name__ == "__main__":
    csv_file_name = os.path.join(constants.OUTPUT_GLOBAL_DIR, "emp_fdr", "MAX", "emp_diff_modules_{dataset}_{algo}.tsv")
    main(dataset="TNFa_2", algo="jactivemodules_greedy", csv_file_name=csv_file_name)
