...
def get_score(dataset_name, score_method):
    if score_method != constants.PREDEFINED_SCORE:
        score_file_name = os.path.join(constants.DATASETS_DIR,dataset_name, "cache", "deg_{}.tsv".format(score_method))
        # if not os.path.exists(score_file_name):
            # DEG_runner.main(dataset_name, method=score_method)
    else:
...
def init_common_params(NETWORK_NAME, dataset_name, score_method = constants.DEG_EDGER):
    ...
    if score_method is not None:
        score_file_name = get_score(dataset_name, score_method)
