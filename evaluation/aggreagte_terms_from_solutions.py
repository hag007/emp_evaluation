...
def main(datasets, algos, prefix, limit=3, hg_th=0.05):
    df_sum=pd.DataFrame()
    ...
    for a in filtered_go_ids:
        if float(df_sum.loc[a,"non_EV"]+df_sum.loc[a,"EV"]) >= limit:
            df_sum.loc[a,"ratio"]=df_sum.loc[a,"non_EV"]/float(df_sum.loc[a,"non_EV"]+df_sum.loc[a,"EV"])
        else:
            df_sum.loc[a,"ratio"]=np.nan

        for cur_alg in algos:
            df_count_matrix.loc[a, cur_alg] = df_non_ev_matrix.loc[a,cur_alg]+df_ev_matrix.loc[a,cur_alg]
            if df_non_ev_matrix.loc[a,cur_alg]+df_ev_matrix.loc[a,cur_alg] >= limit:
    ...
    df_count_matrix.loc[:, "is_sig_in_radalib"] = False
    df_count_matrix.loc[sig_in_null, "is_sig_in_radalib"] = True
    ...
    df_sum.to_csv(os.path.join(constants.OUTPUT_GLOBAL_DIR, "evaluation", "aggregate_solutions_{}_{}.tsv".format(limit, prefix)),sep='\t')
    df_ratio_matrix.to_csv(os.path.join(constants.OUTPUT_GLOBAL_DIR, "evaluation", "aggregate_solutions_ratio_by_algo_{}_{}.tsv".format(limit, prefix)),sep='\t')
    df_count_matrix.to_csv(os.path.join(constants.OUTPUT_GLOBAL_DIR, "evaluation", "aggregate_solutions_count_by_algo_{}_{}.tsv".format(limit, prefix)), sep='\t')
...
if __name__ == "__main__":
    ...
    for limit in limits:
        ...
        prefix="GE"
        main(datasets,algos,prefix,limit)
