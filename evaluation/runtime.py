...
def main(datasets, algos, indices, network, prefix):
    ...
    for cur_ds in datasets:
        for cur_alg in algos:
            for idx in indices:
                timing=get_timing("/specific/netapp5/gaga/hagailevi/emp_test/timings/{}/{}/{}_{}.txt".format(network,cur_ds,cur_alg,idx))

                df_summary.loc["{}_{}_{}".format(cur_ds,cur_alg,idx), "dataset"]=cur_ds
                df_summary.loc["{}_{}_{}".format(cur_ds,cur_alg,idx), "algo"]=cur_alg
                df_summary.loc["{}_{}_{}".format(cur_ds,cur_alg,idx), "index"]=idx
                df_summary.loc["{}_{}_{}".format(cur_ds,cur_alg,idx), "runtime"]=timing
    df_summary.to_csv(os.path.join(constants.OUTPUT_GLOBAL_DIR, "evaluation", "timing_{}_{}.tsv".format(network, prefix)), sep='\t')
