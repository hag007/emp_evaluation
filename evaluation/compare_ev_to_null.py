...
def ehr_for_solution(algo_sample = None, dataset_sample = None,tsv_file_name=None, filtered_go_ids_file=None, sig_in_null_file=None, hg_th=0.05):
    ...
    # OR
    odds1=round(float(tp)/max(fn, 10e-5), 2)
    odds2=round(float(fp)/max(tn, 10e-5), 2)
    OR=round(odds1/max(odds2, 10e-5), 2)
    ...
    print('f1: {}'.format(f1))
    return {"id": "{}_{}".format(algo_sample, dataset_sample), "dataset": dataset_sample, "algo": algo_sample, "EHR": ehr, "precision": precision, "recall": recall, "f1" : f1, "TP": tp, "FP" : fp, "TN": tn , "FN" :fn, "OR" : OR, "pval": sig_score}
...
def main(datasets, algos, prefix, hg_th=0.05):
    ...
    df=df.loc[np.logical_and(df.loc[:, "EHR"]<1, df.loc[:, "EHR"]>0),:]
    df.loc[:,"pval"][df.loc[:,"pval"].isnull()]=1

    for cur_alg in algos:
        df_alg=df[df.loc[:,"algo"] == cur_alg].loc[:, "pval"]
        print(cur_alg)
        fdrs=fdrcorrection0(df_alg)
        df.loc[df_alg.index,"qval"]=fdrs[1]
        df.loc[df_alg.index, "is_significant"] = fdrs[0 ]
    df.loc[:,["TP", "FP", "FN", "TN", "EHR", "f1", "precision", "recall", "TPR", "FPR", "OR", "pval", "qval", "is_significant"]].to_csv(os.path.join(constants.OUTPUT_GLOBAL_DIR, "ev_vs_null_{}.tsv".format(prefix)),sep='\t')
