...
from modules_reader import read_modules
...
def run_pcst(G_cc, i_cc, labels, n_steps, nodes, prize_factor, n_permutations=1000):
    ...
    # print("done generating permuted scores")
    # permuted_prizes = {}
    #
    # for cur_node in nodes:
    #     permuted_prizes[cur_node] = sum(permuted_scores[cur_node])/ float(n_permutations)
    #
    # min_score = min(permuted_prizes.values())
    # max_score = max(permuted_prizes.values())
    # for cur_node in nodes:
    #     permuted_prizes[cur_node] = 0.00001 # 0.00001 # (permuted_prizes[cur_node] - min_score) / float(max(max_score - min_score, 10 ** 5))

    edges_costs = []
    for cur_edge in edges_grid:
        u_score = 0 if G_cc.nodes[nodes[cur_edge[0]]]["pertubed_node"] else 0.9999 # 10-permuted_prizes[nodes[cur_edge[0]]]
        v_score = 0 if G_cc.nodes[nodes[cur_edge[1]]]["pertubed_node"] else 0.9999 # 10-permuted_prizes[nodes[cur_edge[1]]]
    ...
    # print("module {} shrinkage factor: {} ({}/{})".format(i_cc, len(vertices) / float(len(G_cc.nodes)), len(vertices),
    #                                                       len(G_cc.nodes)))
...
def split_subslice_into_putative_modules(G_optimized, improvement_delta, modularity_score_objective, best_modularity):
    ...
    if cur_modularity <= best_modularity + improvement_delta:
        ...
        # print("new: ", len(cur_components), len([a for a in cur_components if len(a) > 3]), best_modularity)
...
def get_putative_modules(G, full_G=None, improvement_delta=0, modularity_score_objective=1, sig_th=0.05, n_cc=1.0):
    ...
    G_optimized.remove_nodes_from(list(nx.isolates(G_optimized)))
    # print("modularity: ", (np.nan if len(G_optimized.nodes)==0 else modularity(G_optimized, [G_optimized.subgraph(c) for c in connected_components(G_optimized)], weight='weight')))
    ...
    # print stats perturbed nodes (commented)
    # for i_cur_cc, n_nodes in enumerate(cc_optimized):
    #
    #     pertubed_nodes_in_cc = [cur_node for cur_node in n_nodes if G.nodes[cur_node]["pertubed_node"]]
...
def analyze_slice(G, cc, i_cc, n_steps, relevant_slices, prize_factor, n_permutations):
    ...
    # print("split into {} modules".format(len(putative_modules_of_slice)))
...
def get_final_modules(G, putative_modules):
    ...
    for i_cur_module, cur_module in enumerate(putative_modules):
        ...
        # print("submodule {} nominal sig score: HG({}, {}, {}, {})={}".format(i_cur_module, len(pertubed_nodes_in_cc),
...
def main(active_genes_file, network_file, slices_file=None, slice_threshold=0.3, module_threshold=0.05, prize_factor=0.7, n_steps=20, n_permutations=1000):
    ...
    # G = mark_extracted_nodes(G)

    modularity_connected_components = read_modules(slices_file)
