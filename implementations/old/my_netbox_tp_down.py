...
def filter_modules(G, connected_components):
    """"""
    pertubed_nodes = []
    for cur_node in G.nodes():
        if G.nodes[cur_node]["pertubed_node"]:
    ...
    for cur_cc in connected_components:
        ...
        sig_scores.append(hypergeom.sf(len(pertubed_nodes_in_cc), len(G.nodes), len(pertubed_nodes),
                                       len(cur_cc)) \
                          + hypergeom.pmf(len(pertubed_nodes_in_cc), len(G.nodes), len(pertubed_nodes),
                                          len(cur_cc)))
        ...
        fdr_bh_results = fdrcorrection0(sig_scores, alpha=0.05, method='indep',
                                        is_sorted=False)
        print "adjust_sig_scores: {}\n(n={})".format(fdr_bh_results[0], sum(fdr_bh_results[1]))
...
if __name__ == "__main__":
    ...
    score_file_name = "/specific/netapp5/gaga/hagailevi/evaluation/bnet/datasets/GE_HC12/output/ge_list.txt"
