...
def main(dataset_name="PASCAL_SUM_Breast_Cancer2.G50",
    ...
    for i_cc, cc in enumerate(optimized_connected_components):
        ...
        n_clusters = [1,2,3]  # [a/25.0 for a in np.arange(101)] # [a/100.0 for a in np.arange(101)]
        ys = []
        y_plateaus = []
        y_plateau_values = []
        plateau_value = 0
        ...
        max_score=-1
        max_n_cluster=-1
        for i_n_cluster, n_cluster in enumerate(n_clusters):
            # alpha = 0.1
            edges_costs = []
            # for cur_edge in edges_grid:
            ...
            vertices, edges = pcst_fast.pcst_fast(edges_grid, vertices_prizes, edges_costs, root, n_cluster, pruning,
                                                  verbosity_level)
            pcst_results[n_cluster] = (vertices, edges)
            cur_score=sum([vertices_prizes[cur_vertex] for cur_vertex in vertices]) - sum([edges_costs[cur_edge] for cur_edge in edges])
            print "cur_score: {}".format(cur_score)
            if max_score + 1 < cur_score:
                max_score=cur_score
        ...
        vertices, edges = pcst_results[max_n_cluster]
        print "n_clusters: {}".format(max_n_cluster)
        ...
        modularity_score_objective = -2 # np.log(len(G_pcst.nodes)) / np.log(len(G.nodes)) if len(G_pcst.nodes) > 10 else -1
