...
def optimize_modularity(G, full_G=None, improvement_delta=0, modularity_score_objective=1, sig_th=0.05, n_cc=1.0):
    ...
    print "initial_scores: {}, {}".format(sig_score, len([n for n in G_optimized.nodes if G_optimized.nodes[n]["pertubed_node"]])/float(max(1,len(G_optimized.nodes))))
    reached_optimal_modularity = (sig_score<sig_th and len(G_optimized.nodes)<10) or len(G_optimized.nodes)==0# len([n for n in G_optimized.nodes if G_optimized.nodes[n]["pertubed_node"]])/float(len(G_optimized.nodes))>0.3 # np.log2(len(pertubed_nodes))
    ...
    print "modularity: ", (np.nan if len(G_optimized.nodes)==0 else modularity(G_optimized, list(connected_component_subgraphs(G_optimized)), weight='weight'))
    ...
    for i_cur_cc, cur_cc in enumerate(cc_optimized):
        ...
        print "submodule {} sig score: HG({}, {}, {}, {})={}".format(i_cur_cc, len(pertubed_nodes_in_cc), len(full_G.nodes),
                                                                     len(pertubed_nodes),
...
def main(dataset_name="PASCAL_SUM_Breast_Cancer2.G50", score_file_name="/media/hag007/Data/bnet/datasets/GE_HC12/output/ge_list.txt", network_file_name="dip.sif", modules_file_name=os.path.join(constants.NETWORKS_DIR, "dip_ng_modularity_components.txt"), module_sig_th=0.05):
    ...
    for i_cc, cc in enumerate(optimized_connected_components):
        ...
        pvals = {}
        for cur_node in nodes:
            pvals[cur_node]= (n_interations-sum(permuted_scores[cur_node]))/float(n_interations) # (n_interations- sum(permuted_scores[cur_node]))/float(n_interations) # calc_emp_pval(scores[cur_node]*-1, np.array(permuted_scores[cur_node])*-1)[0]
        ...
        vertices_prizes=[]
        for cur_node in nodes:
            vertices_prizes.append(G.nodes[cur_node]["pertubed_node"] if G.nodes[cur_node]["pertubed_node"] else scores[cur_node]) # 0*pvals[cur_node]*
        ...
        for cur_edge in edges_grid:
            ...
            edges_costs_original.append(np.min([u_score, v_score])) # min(pvals.values())
        ...
        print "module {} shrinkage factor: {} ({}/{})".format(i_cc, len(vertices)/float(len(G_cc.nodes)), len(vertices), len(G_cc.nodes))
        ...
        module_optimized, sub_modules = optimize_modularity(G_pcst, G, improvement_delta=10 ** -2, modularity_score_objective=modularity_score_objective, n_cc=len(optimized_connected_components))
...
if __name__=="__main__":
    all_qvals=[] #
    datasets=["PASCAL_SUM_Breast_Cancer.G50", "PASCAL_SUM_Crohns_Disease.G50","PASCAL_SUM_Schizophrenia.G50","PASCAL_SUM_Triglycerides.G50", "PASCAL_SUM_Type_2_Diabetes.G50"] # PASCAL_SUM_Breast_Cancer.G50
