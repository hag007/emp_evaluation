...
def main(dataset_name="PASCAL_SUM_Breast_Cancer2.G50",
    ...
    N_STEPS = 20
    module_index=0
    for i_partition, G_partition in enumerate(optimized_connected_components):

        pertubed_nodes_in_partition = [cur_node for cur_node in G_partition if G.nodes[cur_node]["pertubed_node"]]
        scores = {}
        G_cc = nx.subgraph(G, G_partition)
        ...
        for i_iteration in np.arange(n_interations):
            # print "module {}, bg iteration {}".format(i_partition, i_iteration)
            c_nodes = list(nodes)
        ...
        alphas = [1]  # [a/25.0 for a in np.arange(101)] # [a/100.0 for a in np.arange(101)]
        ys = []
        y_plateaus = []
        y_plateau_values = []
        plateau_value = 0
        ...
        for i_alpha, alpha in enumerate(alphas):
            ...
            for i_cur_edge, cur_edge in enumerate(edges_costs_original):
                edges_costs.append(edges_costs_original[i_cur_edge] ** alpha)  # min(pvals.values())
        ...
        # plt.savefig(os.path.join(constants.OUTPUT_GLOBAL_DIR, "alphas_m_{}.png".format(i_partition)))
        ...
        # print "module {} shrinkage factor: {} ({}/{})".format(i_partition, sum(fdr_results[0])/float(len(fdr_results[0])), sum(fdr_results[0]), len(fdr_results[0]))

        print "module {} shrinkage factor: {} ({}/{})".format(i_partition, len(vertices) / float(len(G_cc.nodes)),
                                                              len(vertices), len(G_cc.nodes))

        module_by_gene_id = [nodes[v] for v in vertices]
        print "splitting module {}".format(i_partition)
        G_pcst = nx.Graph()
        ...
        print "split into {} modules".format(len(sub_modules))


        for cur_sub_module in sub_modules:

            modules_to_report.append(list(cur_sub_module.nodes))
            pertubed_nodes_in_module = [cur_node for cur_node in modules_to_report[-1] if G.nodes[cur_node]["pertubed_node"]]
            pertubed_nodes_in_pcst = [cur_node for cur_node in G_pcst.nodes if
                                    G.nodes[cur_node]["pertubed_node"]]

            sig_score = hypergeom.sf(len(pertubed_nodes_in_module), len(G.nodes), len(pertubed_nodes),
                                     len(modules_to_report[-1])) \
                        + hypergeom.pmf(len(pertubed_nodes_in_module), len(G.nodes), len(pertubed_nodes),
                                        len(modules_to_report[-1]))

            pcst_sig_score = hypergeom.sf(len(pertubed_nodes_in_pcst), len(G_partition), len(pertubed_nodes_in_partition),
                                     len(G_pcst.nodes)) \
                        + hypergeom.sf(len(pertubed_nodes_in_pcst), len(G_partition), len(pertubed_nodes_in_partition),
                                     len(G_pcst.nodes))

            print "HG pval ({}, {}, {}, {}) = {}".format(len(pertubed_nodes_in_module), len(G.nodes), len(pertubed_nodes),
                                     len(modules_to_report[-1]), sig_score)

            putative_modules[module_index]= {
                "nodes" : modules_to_report[-1],
                "n_perturbed_nodes" : float(len(pertubed_nodes_in_module)),
                "n_pcst_nodes" : float(len(G_pcst.nodes)),
                "n_pcst_perturbed_nodes" : float(len(pertubed_nodes_in_pcst)),
                "n_partition_nodes": float(len(G_partition)),
                "n_partition_perturbed_nodes": float(len(pertubed_nodes_in_partition)),
                "index" : module_index,
                "pval" : sig_score,
                "pcst_pval": pcst_sig_score,
                "pcst_index" : i_partition,
                "partition_index": i_partition

            }
            module_index+=1

    module_sigs=[]
    for k, v in putative_modules.iteritems():
        v["corrected_pval"]=v["pval"] * len(putative_modules)
        v["corrected_pcst_pval"]=v["pcst_pval"] * len(putative_modules)

        print "density proportion: {}".format((v["n_perturbed_nodes"]/len(v["nodes"])) /\
              (v["n_pcst_perturbed_nodes"]/v["n_pcst_nodes"]))
        print "corrected pval: {}".format(v["corrected_pval"])
        print "pval: {}".format(v["pval"])
        print "pcst corrected pval: {}".format(v["corrected_pcst_pval"])
        print "pcst_pval: {}".format(v["pcst_pval"])

        if v["corrected_pcst_pval"] > 0.05:
            continue
        # if len(v["pcst_index"]==a["pcst_index"] for a in putative_modules.values()) == 1:
        module_sigs.append((k,v))


    module_sigs = sorted(module_sigs, key=lambda a: a[1]["corrected_pval"])
    return [b["nodes"] for a, b in module_sigs]
...
if __name__ == "__main__":
    ...
    datasets = ["PASCAL_SUM_Breast_Cancer.G50"] # , "PASCAL_SUM_Type_2_Diabetes.G50", "PASCAL_SUM_Crohns_Disease.G50","PASCAL_SUM_Schizophrenia.G50","PASCAL_SUM_Triglycerides.G50"]
    for cur_ds in datasets:
        res = main(dataset_name=cur_ds,
                          score_file_name="/media/hag007/Data/bnet/datasets/{}/output/ge_list.txt".format(cur_ds),module_sig_th=0.3)
