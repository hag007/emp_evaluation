...
def run_pcst(G_cc, i_cc, labels, n_steps, nodes, prize_factor, n_permutations=1000):
    ...
    for cur_edge in edges_grid:
        u_score = 0 if G_cc.nodes[nodes[cur_edge[0]]]["pertubed_node"] else permuted_prizes[nodes[cur_edge[0]]]
        v_score = 0 if G_cc.nodes[nodes[cur_edge[1]]]["pertubed_node"] else permuted_prizes[nodes[cur_edge[1]]]

        edges_costs.append(np.max([u_score, v_score]))
...
def get_putative_modules(G, full_G=None, improvement_delta=0, modularity_score_objective=1, sig_th=0.05, n_cc=1.0):
    ...
    while not break_loop:
        break_loop=split_subslice_into_putative_modules(G_optimized, improvement_delta, modularity_score_objective, best_modularity)
